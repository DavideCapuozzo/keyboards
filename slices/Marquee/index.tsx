    ...
    <div className="flex items-center bg-gray-200 py-10 whitespace-nowrap">
      {slice.primary.phrases.map((item, i) => (
        <Fragment key={i}>
          ...
          <div className="text-[180px] md:text-[260px] [text-box:trim-both_cap_alphabetic] font-bold-slanted px-14 text-gray-400/80 leading-none uppercase">{item.text}</div>
          <LogoMark className="size-36 shrink-0"></LogoMark>
        </Fragment>
      ))}
